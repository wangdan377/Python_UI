...
class Test_Prime_pay(unittest.TestCase):
    ...
    def test_prime_pay01(self):
        ...
        self.login.click_File_me()
        eles = self.fun.is_element_exist(self.login.File_tv_login)  # 登录
        # eles = self.driver.find_elements_by_xpath("//*[@text='登录']")  # 登录        #第二种方法，易暴露
        time.sleep(2)
        if eles :
            self.logins.login_01()
            self.prime.click_File_prime01()
            eles = self.fun.is_element_exist(self.prime.File_opens02)  # 立即续费
            if eles:
                ...
                self.assertTrue(self.fun.is_element_exist(self.prime.File_meal01))     #断言套餐一是否存在
                self.members.prime_user01()  # 选择套餐3 进行开通
                self.prime_pays.primes_19()  # 开通页面素材
        else:
            self.prime.click_File_prime01()
            ...
            if eles:
                self.prime_pays.primes_20()  # 续费页面 有效期/素材
                self.assertTrue(self.fun.is_element_exist(self.prime.File_opens02))
                self.prime.click_File_opens02()  # 续费
