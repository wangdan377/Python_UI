...
send_mail = SendEmail(
        ...
        title='API自动化测试报告邮件',
        content='测试发送邮件',
        file=r'D:\API\interfaceTest\result\report.html',
        ssl=True,
    )
    ...
    def run(self):
        ...
        try:
            ...
            if suit is not None:#判断test_suite是否为空
                ...
                #调用HTMLTestRunner
                runner = HTMLTestRunnerCN3_pie_chart_screen.HTMLTestRunner(stream=fp, title='Test Report', description='Test Description')
                runner.run(suit)
            else:
